...
def install_if_necessary(directory: Path):
    """Install npm modules if they're missing."""
    NODE_MODULES = directory / "node_modules"
    if not NODE_MODULES.exists():
        print("Running npm install...")
        subprocess.check_call(
            ["npm", "--prefix", str(directory), "install"])


def bundle_source(entry: Path) -> None:
    ...
    package_json_dir = Path(__file__).parent.parent / "react"

    # Install npm modules if necessary
    install_if_necessary(package_json_dir)
